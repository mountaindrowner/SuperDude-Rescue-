...
| `claude/portion-control-vslice` | ACTIVE side project | **PORTION CONTROL** — the next expansion: kid-friendly Vampire Survivors-style roguelite, self-contained in `games/portion-control/` (own COMPENDIUM.md design doc + HANDOVER.md session state — read those there, not here). Isolate-then-fold-in path, like the Element Lab was for 2.0. Do not develop it on this branch. |
